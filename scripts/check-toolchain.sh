...
read_docker_go_version() {
  local root=$1 line
  while IFS= read -r line; do
    if [[ $line =~ ^FROM[[:space:]]+golang:([0-9]+\.[0-9]+\.[0-9]+)([[:space:]]|$) ]]; then
      printf '%s\n' "${BASH_REMATCH[1]}"
      return 0
    fi
...
check_root() {
  ...
  local go_mod tool_go docker_go tool_pnpm package_pnpm tool_go_minor docker_go_minor
  ...
  printf 'toolchain ok: Go minimum %s, pinned Go %s, pnpm %s\n' "$go_mod" "$tool_go" "$tool_pnpm"
}
...
self_test() {
  ...
  printf 'FROM golang:1.26.6 AS builder\n' > "$tmp/Dockerfile"
  printf '{\n  "packageManager": "pnpm@10.14.0"\n}\n' > "$tmp/package.json"

  check_root "$tmp" >/dev/null

  printf 'golang 1.27.0\npnpm 10.14.0\n' > "$tmp/.tool-versions"
