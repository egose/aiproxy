...
`)
	...
	if err := runModels(context.Background(), cfg, true, "anthropic", false, &stdout, &stderr); err != nil {
...
`)
	var stdout, stderr bytes.Buffer
	err := runModels(context.Background(), cfg, true, "nope", false, &stdout, &stderr)
	if err == nil {
...
`)
	var stdout, stderr bytes.Buffer
	err := runModels(context.Background(), cfg, true, "", false, &stdout, &stderr)
	if err == nil {
